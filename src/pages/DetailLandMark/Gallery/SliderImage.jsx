        ...
        <div className="w-full h-screen bg-black relative flex justify-center items-center">
            <div className="flex flex-col items-center gap-5">
                ...
                <div className="w-[900px] h-[540px]">
                    <img className="w-full h-full object-cover" src={`https://vie-wander-backend.onrender.com/public/images/landmarks/${imagesData[index]}`}></img>
                </div>
                <div className="flex gap-3">
                    {imagesData.map((n, i) => <div key={i} onClick={() => setIndex(i)} className={`cursor-pointer ${i == index ? 'opacity-100' : 'opacity-30'}`}>
                        <img className="w-[100px] h-[80px]" src={`https://vie-wander-backend.onrender.com/public/images/landmarks/${imagesData[i]}`}></img>
                    </div>)}
                </div>
            </div>
